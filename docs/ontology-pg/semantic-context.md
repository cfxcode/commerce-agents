...
The static `preflight-semantics` matrix remains 135 cases. It does not prove the
complete guide request fits. `preflight-guidance` now uses the runtime's exact
`task_payload`, `guidance_input` and provider JSON serializer with typed observations,
plans and recent events. It tests 810 structural combinations: 675 ordinary payloads
and 135 intentionally oversized payloads that must be rejected. These are synthetic
Cartesian stress inputs, not 810 reachable business states or model-accuracy tasks.

```bash
python -m commerce_reasoning.cli preflight-guidance \
  --config configs/experiments/p-closure.yaml \
  --out runtime/acceptance/full-guidance.json
```

`evals/commerce_reasoning/semantic/definition-oracle.v1.json` contains expected sets
transcribed independently from the specification, not generated by the builder.
Reports aggregate expected/included required definitions, state coverage, unrelated
definitions, full payload bytes, solver request bytes and construction fallback rate.
Coverage is null when evidence is missing; an empty report is not 100% coverage.
A successful static build followed by a full-payload overflow is one failed attempt,
not two attempts and not a model call. Coverage describes constructed definitions,
while fallback separately records whether the composed input was usable.

Browser acceptance uses the real retail UI, SSE, session routes, controlled executor,
approval/discard card actions and simulated inventory, with scripted model responses.
It covers English/Chinese, desktop/mobile, historical-context labelling and budget
failure. It is not a real-model test. `.github/workflows/semantic-browser.yml` saves
screenshots and structured results; no production key is required. One last successful
static context per task can be displayed as history, but is not injected again.

Live validation requires `ANTHROPIC_API_KEY` and `COMMERCE_MODEL` in the process;
a non-Claude model also requires `ANTHROPIC_BASE_URL`. In Actions, use repository
Secrets for the key and endpoint and a Variable or Secret for the model. Never put
these values in source, reports, comments or screenshots.

```bash
python -m commerce_reasoning.live_acceptance --check-only
# At most 500 requests, 2,000,000 returned tokens and 30 minutes; no SDK retries.
# No monetary price estimate is made. At most one in-flight request can exceed
# the cumulative returned-token cap because usage arrives after the response.
python -m commerce_reasoning.live_acceptance --validation \
  --out runtime/live-semantic-acceptance
```

The runner does three public smoke cases per mode before paired held-out validation.
Failure, missing usage, failed prerequisites and a rejected comparison are not passed
validation. Status 78 means missing external configuration; it must not be replaced
by scripted results. Publication still requires the separate `verify-semantics`,
`compare-semantics` and `publish-semantic` evidence and reviewer steps. Neither the
runner nor its workflow merges a PR, modifies active, or bypasses release gates.
